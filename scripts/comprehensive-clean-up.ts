...
// Comprehensive function to clean content - keep only Hindi text, spaces, and punctuation
function cleanContent(content: string): string {
  // Remove all possible combinations of problematic symbols
  let cleanContent = content
    ...
    // Remove all combinations of problematic symbols
    .replace(/\/:\s*>/g, '')                   // Remove "/: >"
    .replace(/>\/\s*--\s*:?\s*>\s*--\s*\/?:?>/g, '') // Remove /> -- /: --> >:
...
// Function to comprehensively clean all post bodies
async function comprehensiveCleanUp() {
  try {
    console.log('Comprehensive cleaning of all post bodies...');
    
    // Fetch all posts from Sanity
    const sanityPosts = await client.fetch('*[_type == "post"]{ _id, title }');
    
    console.log(`Found ${sanityPosts.length} posts to comprehensively clean.`);
    
    // Update each post with comprehensively cleaned content
    for (const sanityPost of sanityPosts) {
      console.log(`Comprehensively cleaning post: ${sanityPost.title}`);
      ...
      if (currentPost && currentPost.body && currentPost.body.length > 0) {
        ...
        if (currentText.trim()) {
          ...
          console.log(`  Comprehensively cleaned body for: ${sanityPost.title}`);
        }
      }
    }
    
    console.log('Comprehensive cleaning completed!');
  } catch (error) {
