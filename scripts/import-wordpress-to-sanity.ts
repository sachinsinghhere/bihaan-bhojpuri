...
function htmlToSanityBlocks(html: string): any[] {
  ...
  paragraphs.forEach(p => {
    ...
    // Skip empty paragraphs and contact info
    if (!text || /(?:M[:\s]*\d+|mobile[:\s]*\d+|contact[:\s]*\d+|\d{10})/i.test(text)) {
      return;
    }

    blocks.push(createSanityBlock(text));
  });

  // If no blocks were created, add at least one with the raw content
  if (blocks.length === 0 && html.trim()) {
    // Remove CDATA tags and other WordPress formatting
    const cleanHtml = html.replace(/<!\[CDATA\[(.*?)\]\]>/gs, '$1')
                          .replace(/<!--\s*\/?wp:[^>]*-->/g, '')
                          .replace(/<[^>]*>/g, ' ')
                          .replace(/\s+/g, ' ')
                          .trim();
    ...
    if (cleanHtml) {
      blocks.push(createSanityBlock(cleanHtml));
    }
  }
...
function determineCategory(title: string, content: string): string {
  ...
  const poemIndicators = ['गीत', 'कविता', 'शेर', 'शायरी', 'verses', 'poem', 'poetry'];
  const storyIndicators = ['कहानी', 'story', 'tale', 'narrative'];
...
async function uploadToSanity(posts: SanityPost[]) {
  console.log(`Starting to upload ${posts.length} posts to Sanity...`);
  ...
  for (const post of posts) {
    try {
      ...
      if (existingPost) {
        console.log(`Post with slug "${post.slug.current}" already exists, skipping...`);
        continue;
      }
...
async function main() {
  try {
    ...
    console.log(`Processed ${posts.length} posts for upload.`);

    // Uncomment the next line when ready to upload
    await uploadToSanity(posts);

    console.log('Preview of posts to be uploaded:');
