...
// Function to clean content - remove all specified symbols including >
function cleanContent(content: string): string {
  // Remove all specified symbols and combinations including >
...
// Function to remove > symbol from all post bodies
async function removeGreaterThanSymbol() {
  try {
    console.log('Removing > symbol from all post bodies...');
    
    // Fetch all posts from Sanity
    const sanityPosts = await client.fetch('*[_type == "post"]{ _id, title }');
    
    console.log(`Found ${sanityPosts.length} posts to clean.`);
    
    // Update each post with cleaned content
    for (const sanityPost of sanityPosts) {
      console.log(`Cleaning post: ${sanityPost.title}`);
      ...
      if (currentPost && currentPost.body && currentPost.body.length > 0) {
        ...
        if (currentText.trim()) {
          ...
          console.log(`  Removed > symbol from body for: ${sanityPost.title}`);
        }
      }
    }
    
    console.log('> symbol removal completed!');
  } catch (error) {
    console.error('Error in > symbol removal:', error);
  }
}

removeGreaterThanSymbol();