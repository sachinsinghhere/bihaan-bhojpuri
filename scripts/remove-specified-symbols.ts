...
// Function to remove specified symbols from all post bodies
async function removeSpecifiedSymbols() {
  try {
    console.log('Removing specified symbols from all post bodies...');
    ...
    for (const sanityPost of sanityPosts) {
      ...
      if (currentPost && currentPost.body && currentPost.body.length > 0) {
        ...
        if (currentText.trim()) {
          ...
          console.log(`  Cleaned symbols from body for: ${sanityPost.title}`);
        }
      }
    }
